...
pub struct Smriti {
    ...
    /// Last consolidation timestamp, for cooldown gating.
    /// Prevents consolidate storms on burst writes.
...
impl Smriti {
    ...
    pub fn with_store(store: Arc<dyn Store>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            last_consolidated: std::time::Instant::now(),
            #[cfg(feature = "embeddings")]
            embedder: None,
            #[cfg(feature = "embeddings")]
    ...
    /// Reconsolidate an existing memory with new tags based on the
    /// context in which it was just used. This mimics neural plasticity,
    /// forging new synaptic edges dynamically.
    ...
    pub fn consolidate(&mut self) -> Result<ConsolidationReport> {
        ...
        // Dense cache is populated lazily on first access via dense_for().
        // The eager prewarm was removed because it ran O(N) MiniLM
        // inference (~2ms/memory) on every consolidation. In a 10k-memory
        // database that's ~20s of CPU per consolidation. Lazy population
        // means the first recall after consolidation pays the cost, not
        // the consolidate itself ,  which is the right trade-off for a
        // memory engine.
        Ok(report)
    }
    ...
    /// Recall a causal/temporal trajectory (Episodic Replay) starting
    /// from a node.
    ...
    /// Import memories and edges from a remote peer, applying
    /// Last-Write-Wins (LWW) conflict resolution based on node `version`
    /// and `last_accessed_at`.
    pub fn import_sync_state(
        &mut self,
        mut nodes: Vec<MemoryNode>,
        edges: Vec<(Uuid, Uuid, MemoryEdge)>,
    ) -> Result<()> {
    ...
    /// Auto-consolidate if the hippocampus is filling up, but at most
    /// once per 30 seconds to prevent consolidate storms on burst writes.
    fn maybe_auto_consolidate(&mut self) -> Result<()> {
        if self.hippo.len() >= self.auto_consolidate_threshold
            && self.last_consolidated.elapsed() >= std::time::Duration::from_secs(30)
        {
            self.consolidate()?;
            self.last_consolidated = std::time::Instant::now();
        }
        Ok(())
    }
}
...
impl<'a> RememberBuilder<'a> {
    ...
    pub fn commit(self) -> Result<Uuid> {
        ...
        // Auto-consolidate if buffer is filling up (cooldown-gated).
...
impl<'a> RecallBuilder<'a> {
    ...
    pub fn execute(self) -> Result<RecallResult> {
        ...
        // Semantic Priming: Decay activation across the graph after
        // every recall.
...
mod tests {
    ...
    fn sync_state_roundtrip_lww() {
        ...
        // We will simulate a version bump manually by loading, mutating,
        // and importing.
    ...
    fn clear_activation_resets_priming_between_recalls() {
        ...
        assert!(
            !r.hits.is_empty(),
            "recall after clear_activation must still work"
        );
